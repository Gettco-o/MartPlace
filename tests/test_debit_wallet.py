...
def test_debit_wallet_success():
      ...
      credit_use_case = CreditWallet(wallet_repository=wallet_repo, tenant_repository=tenant_repo, event_bus=fake_bus)
      debit_use_case = DebitWallet(wallet_repository=wallet_repo, tenant_repository=tenant_repo, event_bus=fake_bus)
      ...
      credit_use_case.execute(tenant.id, "user_1", Money(100))
      wallet = debit_use_case.execute(tenant.id, "user_1", Money(40))
...
def test_debit_wallet_insufficient_funds():
      ...
      credit_use_case.execute(tenant.id, "user_1", Money(10))

      with pytest.raises(InsufficientFundsError):
            debit_use_case.execute(tenant.id, "user_1", Money(20))
...
def test_debit_wallet_invalid_amount():
      ...
      with pytest.raises(InvalidAmountError):
            debit_use_case.execute(tenant.id, "user_1", Money(-5))
